...
pub async fn lookup_indicators(configuration: &configuration::Configuration, body: models::RestapiPeriodIndicatorsLookupRequest) -> Result<models::RestapiPeriodLookupIndicatorResponse, Error<LookupIndicatorsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body = body;

    let uri_str = format!("{}/intelligence/combined/lookup-indicators/v1", configuration.base_path);
...
pub async fn search_indicators(configuration: &configuration::Configuration, body: models::RestapiPeriodIndicatorsQueryRequest, sort: Option<&str>, filter: Option<&str>, limit: Option<i32>, offset: Option<&str>) -> Result<models::RestapiPeriodIndicatorResponse, Error<SearchIndicatorsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body = body;
    let p_sort = sort;
    let p_filter = filter;
    ...
    let uri_str = format!("{}/intelligence/combined/indicators/v1", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
