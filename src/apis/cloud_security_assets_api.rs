...
/// struct for typed errors of method [`cloud_security_assets_combined_compliance_by_account`]
...
/// struct for typed errors of method [`cloud_security_assets_entities_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
/// struct for typed errors of method [`cloud_security_assets_queries`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn cloud_security_assets_combined_compliance_by_account(configuration: &configuration::Configuration, filter: Option<&str>, sort: Option<&str>, limit: Option<i32>, offset: Option<i32>, after: Option<&str>, include_failing_iom_severity_counts: Option<bool>) -> Result<models::AssetsPeriodComplianceResponse, Error<CloudSecurityAssetsCombinedComplianceByAccountError>> {
    ...
    if let Some(ref param_value) = p_include_failing_iom_severity_counts {
        req_builder = req_builder.query(&[("include_failing_iom_severity_counts", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CloudSecurityAssetsCombinedComplianceByAccountError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn cloud_security_assets_entities_get(configuration: &configuration::Configuration, ids: Option<Vec<String>>) -> Result<models::AssetsPeriodGetResourcesResponse, Error<CloudSecurityAssetsEntitiesGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_ids = ids;

    let uri_str = format!("{}/cloud-security-assets/entities/resources/v1", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CloudSecurityAssetsEntitiesGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn cloud_security_assets_queries(configuration: &configuration::Configuration, after: Option<&str>, filter: Option<&str>, sort: Option<&str>, limit: Option<i32>, offset: Option<i32>) -> Result<models::AssetsPeriodGetResourceIdsResponse, Error<CloudSecurityAssetsQueriesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_after = after;
    let p_filter = filter;
    let p_sort = sort;
    ...
    let uri_str = format!("{}/cloud-security-assets/queries/resources/v1", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CloudSecurityAssetsQueriesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
