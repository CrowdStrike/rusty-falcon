...
pub async fn download_file(
    ...
    local_var_req_builder =
        local_var_req_builder.query(&[("file_version", &file_version.to_string())]);
...
pub async fn enumerate_file(
    ...
    if let Some(ref local_var_str) = file_name {
        local_var_req_builder =
            local_var_req_builder.query(&[("file_name", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = file_version {
        local_var_req_builder =
            local_var_req_builder.query(&[("file_version", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = platform {
        local_var_req_builder =
            local_var_req_builder.query(&[("platform", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = os {
        local_var_req_builder = local_var_req_builder.query(&[("os", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<EnumerateFileError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
