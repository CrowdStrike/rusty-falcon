...
pub async fn download_file(configuration: &configuration::Configuration, file_name: &str, file_version: &str) -> Result<models::CommonEntitiesResponse, Error<DownloadFileError>> {
...
pub async fn enumerate_file(configuration: &configuration::Configuration, file_name: Option<&str>, file_version: Option<&str>, platform: Option<&str>, os: Option<&str>, arch: Option<&str>, category: Option<&str>) -> Result<models::CommonEntitiesResponse, Error<EnumerateFileError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_file_name = file_name;
    let p_query_file_version = file_version;
...
pub async fn fetch_files_download_info(configuration: &configuration::Configuration, filter: Option<&str>, sort: Option<&str>) -> Result<models::CommonEntitiesResponse, Error<FetchFilesDownloadInfoError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_filter = filter;
    let p_query_sort = sort;

    let uri_str = format!("{}/csdownloads/combined/files-download/v1", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
