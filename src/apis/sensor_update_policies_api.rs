...
pub async fn query_sensor_update_kernels_distinct(configuration: &configuration::Configuration, distinct_field: &str, filter: Option<&str>, offset: Option<i32>, limit: Option<i32>) -> Result<models::MsaPeriodQueryResponse, Error<QuerySensorUpdateKernelsDistinctError>> {
    ...
    let uri_str = format!("{}/policy/queries/sensor-update-kernels/{distinct_field}/v1", configuration.base_path, distinct_field=crate::apis::urlencode(p_distinct_field));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
