...
pub async fn fdrschema_period_entities_period_field_period_get(configuration: &configuration::Configuration, ids: Option<Vec<String>>) -> Result<models::SchemaPeriodSensorFieldResponseV1, Error<FdrschemaPeriodEntitiesPeriodFieldPeriodGetError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<FdrschemaPeriodEntitiesPeriodFieldPeriodGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn fdrschema_period_queries_period_field_period_get(configuration: &configuration::Configuration, limit: Option<i32>, offset: Option<i32>, filter: Option<&str>, sort: Option<&str>) -> Result<models::MsaspecPeriodQueryResponse, Error<FdrschemaPeriodQueriesPeriodFieldPeriodGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_limit = limit;
    let p_offset = offset;
    let p_filter = filter;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<FdrschemaPeriodQueriesPeriodFieldPeriodGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
