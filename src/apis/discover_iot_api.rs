...
pub async fn get_iot_hosts(configuration: &configuration::Configuration, ids: Vec<String>) -> Result<models::DomainDiscoverApiioTHostEntitiesResponse, Error<GetIotHostsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::DomainDiscoverApiioTHostEntitiesResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn query_iot_hosts(configuration: &configuration::Configuration, offset: Option<i32>, limit: Option<i32>, sort: Option<&str>, filter: Option<&str>) -> Result<models::MsaspecQueryResponse, Error<QueryIotHostsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
...
pub async fn query_iot_hosts_v2(configuration: &configuration::Configuration, after: Option<&str>, limit: Option<i32>, sort: Option<&str>, filter: Option<&str>) -> Result<models::DomainDiscoverApiResponse, Error<QueryIotHostsV2Error>> {
    // add a prefix to parameters to efficiently prevent name collisions
