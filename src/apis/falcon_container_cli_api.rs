...
pub async fn read_image_vulnerabilities(configuration: &configuration::Configuration, body: models::ApiPeriodImageLookupRequest) -> Result<models::CorePeriodEntitiesResponse, Error<ReadImageVulnerabilitiesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body = body;

    let uri_str = format!("{}/image-assessment/combined/vulnerability-lookups/v1", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ReadImageVulnerabilitiesError> = serde_json::from_str(&content).ok();
