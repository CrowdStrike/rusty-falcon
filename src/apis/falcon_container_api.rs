...
pub async fn get_credentials(configuration: &configuration::Configuration, ) -> Result<models::DomainPeriodRegistryCredentialsResponse, Error<GetCredentialsError>> {

    let uri_str = format!("{}/container-security/entities/image-registry-credentials/v1", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetCredentialsError> = serde_json::from_str(&content).ok();
