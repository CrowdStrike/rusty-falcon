...
pub async fn download_feed_archive(configuration: &configuration::Configuration, feed_item_id: &str) -> Result<(), Error<DownloadFeedArchiveError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_feed_item_id = feed_item_id;

    let uri_str = format!("{}/indicator-feed/entities/feed-download/v1", configuration.base_path);
...
pub async fn list_feed_types(configuration: &configuration::Configuration, ) -> Result<models::RestapiPeriodIndicatorGetFeedsResponse, Error<ListFeedTypesError>> {

    let uri_str = format!("{}/indicator-feed/entities/feed/v1", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
...
pub async fn query_feed_archives(configuration: &configuration::Configuration, feed_name: &str, feed_interval: &str, since: Option<&str>) -> Result<models::RestapiPeriodIndicatorFeedQueryResponse, Error<QueryFeedArchivesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
