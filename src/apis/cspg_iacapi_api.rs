...
/// struct for typed errors of method [`get_credentials_mixin0`]
...
pub async fn get_credentials_mixin0(configuration: &configuration::Configuration, ) -> Result<models::ModelsPeriodRegistryCredentialsResponse, Error<GetCredentialsMixin0Error>> {

    let uri_str = format!("{}/iac/entities/image-registry-credentials/v1", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetCredentialsMixin0Error> = serde_json::from_str(&content).ok();
