        ...
        match content_type {
            ...
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::CaohuntingapiIntelligenceQueryAggregatesResponse`"
                ))));
            }
        }
    } else {
...
) -> Result<models::CaohuntingapiIntelligenceQueryEntityResponse, Error<GetIntelligenceQueriesError>>
{
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::CaohuntingapiIntelligenceQueryEntityResponse`"
                ))));
            }
        }
    } else {
