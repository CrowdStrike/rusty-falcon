...
/// struct for typed errors of method [`execute_command_proxy`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
/// struct for typed errors of method [`get_combined_plugin_configs`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn execute_command(configuration: &configuration::Configuration, resources: Vec<models::DomainPeriodExecuteCommandV1>) -> Result<models::DomainPeriodExecuteCommandResultsV1, Error<ExecuteCommandError>> {
    ...
    multipart_form = multipart_form.text("resources", p_resources.into_iter().map(|p| p.to_string()).collect::<Vec<String>>().join(",").to_string());
...
pub async fn execute_command_proxy(configuration: &configuration::Configuration, body: models::DomainPeriodExecuteCommandRequestV1) -> Result<serde_json::Value, Error<ExecuteCommandProxyError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body = body;

    let uri_str = format!("{}/plugins/entities/execute-proxy/v1", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ExecuteCommandProxyError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn get_combined_plugin_configs(configuration: &configuration::Configuration, filter: Option<&str>, limit: Option<i32>, offset: Option<i32>, sort: Option<&str>) -> Result<models::DomainPeriodConfigsV1, Error<GetCombinedPluginConfigsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_filter = filter;
    let p_limit = limit;
    let p_offset = offset;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetCombinedPluginConfigsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
