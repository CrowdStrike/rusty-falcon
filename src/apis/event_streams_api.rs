...
pub async fn list_available_streams_o_auth2(configuration: &configuration::Configuration, app_id: &str, format: Option<&str>) -> Result<models::MainPeriodDiscoveryResponseV2, Error<ListAvailableStreamsOAuth2Error>> {
...
pub async fn refresh_active_stream_session(configuration: &configuration::Configuration, action_name: &str, app_id: &str, partition: i32) -> Result<models::MsaPeriodReplyMetaOnly, Error<RefreshActiveStreamSessionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_action_name = action_name;
    let p_app_id = app_id;
    let p_partition = partition;

    let uri_str = format!("{}/sensors/entities/datafeed-actions/v1/{partition}", configuration.base_path, partition=p_partition);
