...
/// struct for typed errors of method [`post_graphql_query_mixin0`]
...
pub async fn post_graphql_query_mixin0(configuration: &configuration::Configuration, ) -> Result<std::collections::HashMap<String, serde_json::Value>, Error<PostGraphqlQueryMixin0Error>> {

    let uri_str = format!("{}/cloud-data-protection/entities/schedules/graphql/v2", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<PostGraphqlQueryMixin0Error> = serde_json::from_str(&content).ok();
