...
/// struct for typed errors of method [`entities_rules_ownership_put_v1`]
...
pub async fn entities_rules_ownership_put_v1(configuration: &configuration::Configuration, body: models::ApiRuleOwnerPutRequestV1) -> Result<models::ApiGetEntitiesRulesResponseV1, Error<EntitiesRulesOwnershipPutV1Error>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_body = body;

    let uri_str = format!("{}/correlation-rules/entities/rules/ownership/v1", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<EntitiesRulesOwnershipPutV1Error> = serde_json::from_str(&content).ok();
